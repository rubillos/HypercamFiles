...
class SimpleLineProtocol:
	...
	def read_line(self, timeout=0.5):
		...
		while time.time() < end_time and len(line) == 0:
			readable, writable, exceptional = select.select([self.socket], [], [], 0.05)
			...
			if len(readable) > 0:
				...
				self.buffer = self.buffer + d.replace('\\n', '\n').replace('\r', '\n')
				end_time = time.time() + timeout
		...
		if len(line) == 0 and len(self.buffer) > 0:
			line = self.buffer
			self.buffer = b''

		return line

	def read_json_line(self, timeout=0.5):
		end_time = time.time() + timeout
		line = b''
		json_data = None

		while time.time() < end_time and b'{' not in line and b'}' not in line:
	...
	def read_data(self, timeout=0.5):
		json_data = None
		message = None
...
def play_sound(sound_name):
	if debugging:
		log_print("Play sound: \'" + sound_name + "\'")
		...
		try:
			...
			if sock:
				...
				while sock:
					...
					try:
						new_data,message = conn.read_data()
							...
							if status == 'P':
								...
								if last_baby_step == -1000 and current_baby_step == 0:
									...
									if current_baby_step > last_baby_step:
										play_sound("bubble-down.wav")
									else:
										play_sound("bubble-up.wav")
					...
					if status == 'P' and not timelapse_folder:
						conn.write('M36')
						file_info = conn.read_json_line(timeout=1.0)
...
def button_down(channel):
	...
	if printing:
		...
		conn.write('M290 S' + str(baby_step_amount))
...
if __name__ == "__main__":
	...
	try:
		...
		GPIO.setup(button_down_ground_pin, GPIO.OUT, initial=GPIO.LOW)
		GPIO.add_event_detect(button_down_pin, GPIO.FALLING, callback=button_down, bouncetime=200)

		GPIO.setup(button_up_pin, GPIO.IN, pull_up_down=GPIO.PUD_UP)
		GPIO.setup(button_up_ground_pin, GPIO.OUT, initial=GPIO.LOW)
		GPIO.add_event_detect(button_up_pin, GPIO.FALLING, callback=button_up, bouncetime=200)
